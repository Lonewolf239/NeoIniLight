...
| `SetValues` | Установка/создание нескольких пар ключ-значение | `SetValuesAsync` |
...
**Встроенные пресеты:** `Default`, `Safe`, `Performance`, `BufferedAutoSave(interval)`.
...
> **Примечание:** Ошибки на уровне провайдера (файловый I/O) репортятся через событие `Error`; если на него никто не подписан, вместо этого выбрасывается исходное исключение. `AddKey`, `RenameKey` и `RenameSection` (и их async-версии) всегда выбрасывают `InvalidOperationException` при конфликте — они дополнительно уведомляют `Error`, если на него есть подписчик, но подписка не подавляет исключение.

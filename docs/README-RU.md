...
Лёгкая, потокобезопасная библиотека конфигурации INI для .NET с атомарной записью и автоматическим резервным копированием.
...
- **Версия:** 1.0 | **.NET 5+** | **.NET Standard 2.0**
...
**Black Box Design** — вся внутренняя логика скрыта за простым публичным API класса `NeoIniDocument`. Вы работаете только с методами и событиями, не задумываясь о деталях реализации.
